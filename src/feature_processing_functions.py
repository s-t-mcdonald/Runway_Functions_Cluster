...
def process_projected_rate(date_range, raw_rate_df, rate_type = 'scheduled_runway_arrival', horizon = 10):
    ...
    raw_rate_df['t_delta'] = ((raw_rate_df[f"{rate_type}_time"] - raw_rate_df['timestamp']).copy(deep=True).astype('timedelta64[s]')/60)
...
def create_train_val_test_masks(complete_df, airport,train_labels, open_submission):
    ...
    prescreened_daterange = (pd.date_range(start=start_date, end=end_date, freq='60T')).to_frame(name = 'timestamp')
